...
if [ "$UNZIP_NEEDED" = "yes" ]; then
    ...
    cd "$FOUNDRY_APP_DIR" || {
        echo "Error: Cannot change to application directory $FOUNDRY_APP_DIR"
        exit 1
    }
    ...
    cp "$ZIP_FILE" ./temp_foundry_zip.zip || {
        echo "Error: Failed to copy zip file. Check permissions on $FOUNDRY_APP_DIR."
        exit 1
    }
    ...
    unzip -o ./temp_foundry_zip.zip ||
        {
            echo "Error: Failed to unzip. Ensure 'unzip' is installed and zip file is valid."
            exit 1
        }
...
cd "$FOUNDRY_NODE_APP_ROOT" || {
    echo "Error: Cannot change to actual application root $FOUNDRY_NODE_APP_ROOT"
    exit 1
}
